...
<div class="max-w-xl mt-2">
    <form method="POST" action="{{ route('suppliers.store') }}" class="space-y-6">
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-4">
            <h2 class="font-semibold text-white">Supplier Details</h2>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="payment_terms_days" value="{{ old('payment_terms_days', 30) }}" min="0"
                           class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500">
                </div>
                <div class="col-span-2">
                    <label class="block text-xs text-slate-400 mb-1">Address</label>
                    <textarea name="address" rows="2" class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm">{{ old('address') }}</textarea>
                </div>
                <div class="col-span-2">
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg text-sm font-medium">Add Supplier</button>
